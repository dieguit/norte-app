...
export function FinancialOnboarding() {
  ...
  const continueFromObjective = () => {
    const result = createObjectiveSchema(currentMonth).safeParse(objectiveForm.state.values)
    if (!result.success) {
      ...
      setTimeout(() => {
        document.querySelector<HTMLElement>('[data-invalid="true"] input, [data-invalid="true"] button')?.focus()
      }, 0)
      return
    }
      ...
      <main className="rounded-2xl border border-[var(--line)] bg-[var(--surface-strong)] p-6 shadow-[var(--shadow-card)] sm:p-8">
        {step === 1 && (
          <section className="flex flex-col gap-6" aria-labelledby="welcome-title">
            <div>
              <h1 id="welcome-title" className="font-serif text-3xl font-bold tracking-tight text-[var(--sea-ink)]">
                Tu plan empieza con una dirección clara
              </h1>
              <p className="mt-2 text-sm leading-relaxed text-[var(--sea-ink-soft)] sm:text-base">
                Norte conecta lo que querés lograr con tus ingresos y gastos para mostrarte un camino posible.
              </p>
            </div>
            <div className="grid gap-3 sm:grid-cols-3">
              <article>
                <h2 className="font-semibold text-[var(--sea-ink)]">Objetivos</h2>
                <p className="mt-1 text-sm text-[var(--sea-ink-soft)]">Definí qué querés alcanzar.</p>
              </article>
              <article>
                <h2 className="font-semibold text-[var(--sea-ink)]">Finanzas</h2>
                <p className="mt-1 text-sm text-[var(--sea-ink-soft)]">Sumá ingresos y gastos para entender tu punto de partida.</p>
              </article>
              <article>
                <h2 className="font-semibold text-[var(--sea-ink)]">Hoja de ruta</h2>
                <p className="mt-1 text-sm text-[var(--sea-ink-soft)]">Seguí cómo cada decisión cambia tu camino.</p>
              </article>
            </div>
            <div className="flex justify-end">
              <Button type="button" onClick={() => setStep(2)}>Empezar</Button>
            </div>
          </section>
        )}
        ...
        {step === 2 && (
          <section className="flex flex-col gap-6" aria-labelledby="objective-title">
            <div>
              <h1 id="objective-title" className="font-serif text-3xl font-bold tracking-tight text-[var(--sea-ink)]">
                Elegí tu primer objetivo
              </h1>
              <p className="mt-2 text-sm text-[var(--sea-ink-soft)]">
                Empezamos por uno. Más adelante vas a poder sumar o cambiar objetivos.
              </p>
            </div>
            <GoalObjectiveFields
              form={objectiveForm}
            ...
            <div className="flex items-center justify-between">
              ...
              <Button type="button" onClick={continueFromObjective}>Continuar</Button>
            </div>
          </section>
        )}
        ...
        {step === 3 && (
          <section className="flex flex-col gap-6" aria-labelledby="income-title">
            <div>
              <h1 id="income-title" className="font-serif text-3xl font-bold tracking-tight text-[var(--sea-ink)]">Ingresos</h1>
              <p className="mt-2 text-sm text-[var(--sea-ink-soft)]">
                Acá vas a registrar de dónde viene tu dinero y cuándo lo recibís.
              </p>
            </div>
        ...
        {step === 4 && (
          <section className="flex flex-col gap-6" aria-labelledby="expenses-title">
            <div>
              <h1 id="expenses-title" className="font-serif text-3xl font-bold tracking-tight text-[var(--sea-ink)]">Gastos</h1>
              <p className="mt-2 text-sm text-[var(--sea-ink-soft)]">
                Acá vas a registrar tus gastos para entender cuánto dinero queda disponible cada mes.
              </p>
            </div>
            <p className="rounded-xl border border-[var(--line)] bg-[var(--foam)] p-4 text-sm text-[var(--sea-ink-soft)]">
              Este paso se completa en la próxima etapa.
