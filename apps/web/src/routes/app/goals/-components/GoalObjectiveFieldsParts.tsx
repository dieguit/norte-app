...
export function GoalTypeField({ form, values, validationErrors, immutableIdentity, hasEmergencyFund }: FieldProps & { hasEmergencyFund: boolean }) {
  ...
  return (
    <Field data-invalid={!!validationErrors.type}>
      ...
      >
        <SelectTrigger id="goal-type-trigger" aria-label="Tipo de objetivo" className="w-full" disabled={immutableIdentity}>
...
export function GoalNameField({ values, validationErrors, form }: FieldProps) {
  return (
    <Field data-invalid={!!validationErrors.name}>
      ...
      <Input id="goal-name-input" aria-label="Nombre del objetivo" placeholder="Ej: Vacaciones 2027, Auto nuevo..." value={values.name} onChange={(event) => form.setFieldValue('name', event.target.value)} />
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <Field data-invalid={!!validationErrors.currency}>
        ...
        <Select value={values.currency} disabled={immutableIdentity} onValueChange={(value) => value && form.setFieldValue('currency', value as never)}>
          <SelectTrigger id="goal-currency-trigger" aria-label="Moneda" className="w-full" disabled={immutableIdentity}><SelectValue>{CURRENCIES.find((option) => option.value === values.currency)?.label ?? 'Seleccionar moneda'}</SelectValue></SelectTrigger>
      ...
      <Field data-invalid={!!validationErrors.targetAmount}>
        ...
        <Input id="goal-target-amount-input" aria-label="Monto objetivo" inputMode="decimal" placeholder="0" value={values.targetAmount} onChange={(event) => form.setFieldValue('targetAmount', formatMoneyInput(event.target.value))} />
        {validationErrors.targetAmount && <FieldError>{validationErrors.targetAmount}</FieldError>}
      </Field>
    </div>
  )
}
...
function AnnualReturnField({ form, values, validationErrors }: FieldProps) {
  return (
    <Field data-invalid={!!validationErrors.annualReturnRate}>
      ...
      <Input id="annual-return-rate-input" aria-label="Rendimiento anual estimado (%)" inputMode="decimal" value={values.annualReturnRate} onChange={(event) => form.setFieldValue('annualReturnRate', event.target.value)} className="w-full sm:w-40" />
      <FieldDescription>Tasa anual esperada para proyectar el crecimiento.</FieldDescription>
      {validationErrors.annualReturnRate && <FieldError>{validationErrors.annualReturnRate}</FieldError>}
    </Field>
  )
}
  ...
  return (
    <Field data-invalid={!!validationErrors.availability}>
      ...
      <Select value={values.availability} onValueChange={(value) => value && form.setFieldValue('availability', value as never)}>
        <SelectTrigger id="availability-select-trigger" aria-label="Disponibilidad de los fondos" className="w-full"><SelectValue>{AVAILABILITIES.find((option) => option.value === values.availability)?.label ?? 'Seleccionar disponibilidad'}</SelectValue></SelectTrigger>
      ...
      {validationErrors.availability && <FieldError>{validationErrors.availability}</FieldError>}
    </Field>
  )
}
...
function AvailableFromField({ form, values, validationErrors, context }: FieldProps & { context: GoalCreationContext }) {
  ...
  return (
    <Field data-invalid={!!validationErrors.availableFromMonth}>
      ...
      <MonthPickerInput id="available-from-month-input" aria-label="Mes a partir del cual estará disponible" value={values.availableFromMonth} minMonth={context.currentMonth} onValueChange={(month) => form.setFieldValue('availableFromMonth', month)} />
      {validationErrors.availableFromMonth && <FieldError>{validationErrors.availableFromMonth}</FieldError>}
    </Field>
  )
}
    ...
    <>
      <Field data-invalid={!!validationErrors.strategy}>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:gap-6">
          <label className="flex cursor-pointer items-center gap-2.5 text-sm font-medium text-[var(--sea-ink)]"><input type="radio" name="strategy" value="save" aria-label="Ahorrar" checked={values.strategy === 'save'} onChange={() => form.setFieldValue('strategy', 'save')} disabled={immutableIdentity} className="size-4 text-[var(--palm)] focus:ring-[var(--palm)]" /><span>Ahorrar</span></label>
          <label className="flex cursor-pointer items-center gap-2.5 text-sm font-medium text-[var(--sea-ink)]"><input type="radio" name="strategy" value="invest" aria-label="Invertir" checked={values.strategy === 'invest'} onChange={() => form.setFieldValue('strategy', 'invest')} disabled={immutableIdentity} className="size-4 text-[var(--palm)] focus:ring-[var(--palm)]" /><span>Invertir</span></label>
        </div>
        {validationErrors.strategy && <FieldError>{validationErrors.strategy}</FieldError>}
      </Field>
