...
export function GoalImpact({
  ...
  const currentAllocations = values.allocations && values.allocations.length > 0
    ? values.allocations
    : preview?.proposal.allocationGroups ?? []

  const handlePercentageChange = (groupKey: string, goalId: string, percentage: string) => {
    const updated = currentAllocations.map((group) => {
      if (group.key !== groupKey) return group
      return {
        ...group,
        entries: group.entries.map((entry) =>
          entry.goalId === goalId ? { ...entry, percentage } : entry,
        ),
      }
    })
  ...
  const groupsToDisplay: GoalCreationAllocationGroup[] = (
    ...
    const draftGroup = values.allocations?.find((g) => g.key === proposalGroup.key)
    const baseEntries = proposalGroup.entries.map((entry) => {
      const draftEntry = draftGroup?.entries.find((e) => e.goalId === entry.goalId)
      return {
        ...entry,
  ...
  const isAllocationsValid =
    ...
    groupsToDisplay.every((group) => calculatePercentageSum(group.entries).isEqualTo(100))

  const isPreviewOutdated = !preview || groupsToDisplay.some((g) => {
    const propGroup = preview.proposal.allocationGroups.find((pg) => pg.key === g.key)
    if (!propGroup) return true
    return g.entries.some((e) => {
      const propEntry = propGroup.entries.find((pe) => pe.goalId === e.goalId)
      if (!propEntry) return true
      return new BigNumber((e.percentage || '0').replace(',', '.')).toFixed(2) !==
        new BigNumber((propEntry.percentage || '0').replace(',', '.')).toFixed(2)
    })
  })
      ...
      <section aria-label="Impacto en objetivos" className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <h3 className="text-base font-semibold text-[var(--sea-ink)]">
            Impacto en las fechas de cumplimiento
          </h3>
